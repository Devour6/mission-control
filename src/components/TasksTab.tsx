      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="px-3 py-1.5 bg-indigo-500 hover:bg-indigo-600 rounded-lg text-xs font-medium transition-colors ml-2"
          >
            + New Task
          </button>
        </div>
      ...
      {showForm && (
        <div className="bg-[#1a1d27] border border-[#2e3345] rounded-xl p-4 mb-6 space-y-3">
          <input
            ...
            className="w-full bg-[#242836] border border-[#2e3345] rounded-lg px-3 py-2 text-sm outline-none focus:border-indigo-500/50"
          />
          <textarea
            placeholder="Description"
            value={form.description}
            onChange={(e) => setForm({ ...form, description: e.target.value })}
            className="w-full bg-[#242836] border border-[#2e3345] rounded-lg px-3 py-2 text-sm outline-none resize-none min-h-[60px] focus:border-indigo-500/50"
          />
          <div className="flex gap-3">
            <select
              value={form.assignee}
              onChange={(e) => setForm({ ...form, assignee: e.target.value as Assignee })}
              className="bg-[#242836] border border-[#2e3345] rounded-lg px-3 py-2 text-sm outline-none"
            >
                ...
                {(provided, snapshot) => (
                  <div
                    ...
                    className={`min-h-[200px] rounded-xl p-2 space-y-2 transition-colors ${
                      snapshot.isDraggingOver ? "bg-indigo-500/5" : "bg-[#1a1d27]/50"
                    }`}
                  >
                    {filtered
                        ...
                        <Draggable key={task.id} draggableId={task.id} index={idx}>
                          {(prov) => (
                            <div
                              ...
                              className={`bg-[#242836] border rounded-lg p-3 group ${
                            ...
                            >
                              ...
                              {task.description && (
                                <p className="text-xs text-[#8b8fa3] mt-1 line-clamp-2">
                                  {task.description}
                                </p>
                              )}
