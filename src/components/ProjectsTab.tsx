          ...
          <div key={key} className="bg-[#1a1d27] border border-[#2e3345] rounded-xl overflow-hidden">
            <div className="px-5 py-4 border-b border-[#2e3345] flex items-center justify-between">
              ...
              {div.status === "coming_soon" && (
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#242836] text-[#8b8fa3]">Coming Soon</span>
              )}
            </div>
            <div className="p-5">
              {div.projects.length === 0 ? (
                ...
                <div className="space-y-3">
                  {div.projects.map((p) => (
                    <div key={p.id} className="flex items-start justify-between bg-[#242836] rounded-lg p-3">
                      <div>
                        <h4 className="text-sm font-medium text-[#e4e6ed]">{p.name}</h4>
                        <p className="text-xs text-[#8b8fa3] mt-0.5">{p.description}</p>
                        {p.members && p.members.length > 0 && (
                          <p className="text-[10px] text-[#8b8fa3] mt-1">{p.members.join(", ")}</p>
                        )}
                      </div>
                      <span className={`text-[10px] px-2 py-0.5 rounded-full shrink-0 ml-3 ${(statusColors[p.status] || statusColors.active).bg} ${(statusColors[p.status] || statusColors.active).text}`}>
                        {p.status}
                      </span>
                    </div>
                  ))}
